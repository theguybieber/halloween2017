...
/*  * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 Guy Bieber - Added parameter to callback
* * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
int8_t NewTimer::every(unsigned long period, int (*callback)(String), int repeatCount, String param)
{
	...
	_events[i].count = 0;
	_events[i].param = param; 
	return i;
}

int8_t NewTimer::every(unsigned long period, int (*callback)(String), String param)
{
	return every(period, callback, -1, param); // - means forever
}

int8_t NewTimer::after(unsigned long period, int (*callback)(String), String param)
{
	return every(period, callback, 1, param);
}
...
void NewTimer::update(unsigned long now)
{
